...
const createDmSchema = z.object({
  userId: z
    .string()
    .cuid({ message: 'ユーザー ID が正しい形式ではありません' }),
});
...
export const POST = withAuth(async (request: NextRequest, _, user: User) => {
  try {
    ...
    if (recipientId === user.id) {
      return NextResponse.json(
        { error: '自分自身とダイレクトメッセージを開始することはできません' },
        { status: 400 }
      );
    }

    // DM チャンネルを作成 （既存のものがあればそれを返す）
    const channel = await channelOperations.createDirectMessage(
      user.id,
      recipientId
    );
