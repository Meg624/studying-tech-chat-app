...
const addMembersSchema = z.object({
  userIds: z
    .array(
      z.string().cuid({ message: 'ユーザー ID が正しい形式ではありません' })
    )
    .min(1, {
...
export const POST = withAuth(
  async (request: NextRequest, { params }: Params, user: User) => {
    // チャンネルIDのバリデーション
    const channelResult = channelIdSchema.safeParse({
      channelId: params.channelId,
    });
    if (!channelResult.success) {
      return NextResponse.json(
        {
          error: channelResult.error.format(),
          message: 'チャンネル ID が無効です',
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!membersResult.success) {
        return NextResponse.json(
          {
            error: membersResult.error.format(),
            message: 'メンバー追加データが不正です',
          },
          { status: 400 }
        );
      }
      ...
      if (newUserIds.length === 0) {
        return NextResponse.json(
          { message: '全てのユーザーは既にチャンネルのメンバーです' },
          { status: 200 }
        );
      }

      // チャンネルにメンバーを追加
      const updatedChannel = await channelOperations.addMembersToChannel(
        channelId,
        newUserIds
      );
