...
export const POST = withAuth(
  async (request: NextRequest, { params }: Params, user: User) => {
    // パラメーターのバリデーション
    const channelIdResult = channelIdSchema.safeParse({
      channelId: params.channelId,
    });
    if (!channelIdResult.success) {
      return NextResponse.json(
        {
          error: channelIdResult.error.format(),
    ...
    try {
      ...
      if (!messageResult.success) {
        return NextResponse.json(
          {
            error: messageResult.error.format(),
      ...
      // メッセージを作成して保存
      const message = await messageOperations.createMessage(
        channelId,
        user.id,
        content
      );
